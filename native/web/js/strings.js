...
export const S = Object.freeze({
  ...
  // ── קובץ ההתקנה של החבילה המלאה ───────────────────────────────────────────
  // מוצג פעם אחת, אחרי שהחבילה פרסה את עצמה והעבירה את השרביט ל-exe
  // הרזה. ⚠️ **שאלה ולא מחיקה**: אותו קובץ עשוי להיות מיועד גם למחשב
  // שני, ומחיקה שקטה של הורדה של ~‎96MB הייתה מאלצת להוריד אותה שוב.
  bundleInstaller: Object.freeze({
    bannerTitle: (size) =>
        `כל התוספים נפרסו. קובץ ההתקנה (${size}) אינו נחוץ עוד`,
    bannerButton: 'מחיקת קובץ ההתקנה',
    bannerDismissTooltip: 'להשאיר אותו',
    deleted: 'קובץ ההתקנה נמחק',
    deleteFailed: (error) => `מחיקת קובץ ההתקנה נכשלה: ${error}`,
  }),
