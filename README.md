...
הוא כותב **שני** פלטים מאותו מקור:

| קובץ | מי מציג אותו |
|---|---|
| `windows/runner/resources/app_icon.ico` | קובץ ההרצה — שורת המשימות, Explorer, Alt-Tab |
| `assets/images/app_icon.png` (256px) | `AppTitleBar` — לצד שם התוכנה **בתוך** החלון |

שניהם מהסקריפט ולא בהעתקה ידנית, כי הם נטו להיפרד: בתחילה ישב בשורת הכותרת
הלוגו של אוצריא שהגיע עם הפורט מהלאנצ'ר, ובעוד ששורת המשימות כבר הציגה את
האייקון החדש — החלון עצמו נראה כמו תוכנה אחרת לגמרי.

ה-stub נושא את **אותו** אייקון (`stub.rc` מצביע לאותו `.ico`), ולכן די
...
1. **ה-stub** (ה-EXE שמפיצים) נושא בתוכו את ערמת הקבצים כ-resource ופורש
   ...
   החילוץ **דורס ואינו מוחק** — זה מכוון, כדי ש-`OtzariaData\` (המראה, הלוג,
   המצב) לא ייפגע. המחיר: קובץ שהוסר מהתוכנה נשאר כשארית יתומה אצל מי שכבר
   הריץ גרסה קודמת. התקנה חדשה נקייה, ושארית כזו אינה נטענת על ידי כלום.
